...
export default function ContractsPage() {
  ...
  useEffect(() => {
    supabase.from('crop_types').select('id, crop_name').eq('is_active', true).order('crop_name')
  ...
  return (
    <CrudPage
      ...
      select="*, crop_types(crop_name)"
      orderBy="contract_date"
      columns={[
        { key: 'company_name', label: '契作廠商' },
        { key: 'contract_no', label: '合約編號' },
        { key: 'crop', label: '作物', render: r => r.crop_types?.crop_name },
        ...
        { key: 'contract_status', label: '狀態', render: r => STATUS[r.contract_status] },
      ]}
      fields={[
        ...
        { name: 'crop_type_id', label: '契作作物', type: 'select',
          options: cropTypes.map(c => ({ value: c.id, label: c.crop_name })) },
        ...
        { name: 'contract_status', label: '合約狀態', type: 'select', required: true,
